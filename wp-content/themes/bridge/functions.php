...
if (!function_exists('bridge_qode_styles')) {
	...
	wp_localize_script(
		...
		array(
			...
			'nonce'         => wp_create_nonce('custom-js-' . $nonceval),
			'swiperpagination' => __('of'),
			'_token' => $nonceval,
		)
	);
...
if (!function_exists('bridge_qode_register_menus')) {
	...
	function bridge_qode_register_menus()
	{
		...
		if ((isset($bridge_qode_options['header_bottom_appearance']) && $bridge_qode_options['header_bottom_appearance'] != "stick_with_left_right_menu") || (isset($bridge_qode_options['vertical_area']) && $bridge_qode_options['vertical_area'] == "yes")) {
			...
			register_nav_menus(
				array(
					'top-navigation' => esc_html__('Top Navigation', 'bridge')
				)
			);
		}
		...
		register_nav_menus(
			array(
				'mobile-navigation' => esc_html__('Mobile Navigation', 'bridge')
			)
		);
		...
		register_nav_menus(
			array(
				'popup-navigation' => esc_html__('Fullscreen Navigation', 'bridge')
			)
		);
		...
		if ((isset($bridge_qode_options['header_bottom_appearance']) && $bridge_qode_options['header_bottom_appearance'] == "stick_with_left_right_menu") && (isset($bridge_qode_options['vertical_area']) && $bridge_qode_options['vertical_area'] == "no")) {
			...
			register_nav_menus(
				array(
					'left-top-navigation' => esc_html__('Left Top Navigation', 'bridge')
				)
			);
			...
			register_nav_menus(
				array(
					'right-top-navigation' => esc_html__('Right Top Navigation', 'bridge')
				)
			);
		}
	}
		...
		if ($pagenow === 'admin.php' && $is_theme_options_page) {
			if (!bridge_qode_assets_folders_writable()) { ?>
				<div class="error">
					...
					<!--					<p>--><?php //esc_html_e('It seams that css and js files in theme folder aren\'t writable.', 'bridge'); 
													?><!--</p>-->
				</div>
			<?php }
		}
...
add_action('wp_ajax_workshop_listing', 'workshop_listing');
add_action('wp_ajax_nopriv_workshop_listing', 'workshop_listing');


function workshop_listing()
{
	...
	if (isset($_POST['_nonce'])) {
		$tokenval = sanitize_text_field(wp_unslash($_POST['_nonce']));
		$workshoplisting_nonce = (! empty($_POST['workshoplisting_nonce'])) ? esc_attr(sanitize_text_field(wp_unslash($_POST['workshoplisting_nonce']))) : '';

		if (! wp_verify_nonce($workshoplisting_nonce, 'custom-js-' . $tokenval)) {
			echo '<div class = "cvf-pagination-content"><h2>' . esc_html_e("Sorry, we weren't able to find records..", 'was-hotels-news') . '</h2></div>';
			exit();
		}
	}
	...
	if (isset($_POST['page'])) {
		...
		$first_btn = true;
		$last_btn = true;
		$order_by = (! empty($order_by) ? ('date' !== $order_by ? $order_by : 'publish_date') : 'publish_date');
		...
		$countquery = new WP_Query($args1);
		$count = $countquery->found_posts;
		$firnalposts = $workshops->posts;


		$msg .= '<div class="tab-content" id="myTabContent">';

		while ($workshops->have_posts()) : $workshops->the_post();
			$msg .= '<div class="workshop-item">';
			if (has_post_thumbnail()) :
				$msg .= '<div class="workshops-feature-image">
                                <a href="' . get_the_permalink() . '"><img src="' . esc_url(get_the_post_thumbnail_url(get_the_ID(), 'full')) . '" alt="' . get_the_title() . '">
                                </a>';

				$dates = get_field('workshop_date', get_the_ID());
				if ($dates) :

					$msg .= '<p class="workshops-dates">
						 ' . date('F j, Y', strtotime($dates)) .
						'</p>';
				endif;
				$msg .= '</div>';
			endif;

			$msg .= '<h3 class="workshops-title"><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h3>';

			$msg .= '<div class="workshop-short-desc">
				<p>' . wp_kses_post(get_the_excerpt()) . '</p>
			</div>';

			$msg .= '<a href="' . get_the_permalink() . '" class="workshops-btn">' . __('Read More') . '</a></div>';
		endwhile;
		...
		if ($first_btn && $cur_page > 1) {
			$pag_container .= "<li p='1' class='active'><a href='" . esc_url($current_site_url . '1/') . "'>First</a></li>";
		} else if ($first_btn) {
			$pag_container .= "<li p='1' class='inactive'><a href='" . esc_url($current_site_url . '1/') . "'>First</a></li>";
		}

		if ($previous_btn && $cur_page > 1) {
			$pre = $cur_page - 1;
			$pag_container .= "<li p='$pre' class='active'><a href='" . esc_url($current_site_url . $pre . '/') . "'>Previous</a></li>";
		...
		for ($i = $start_loop; $i <= $end_loop; $i++) {
			...
			if ((($start_loop + 1)) == $cur_page || (($start_loop + 1) == $i)) {
				$dotclass = '';
			} else {
				$dotclass = "<span class='dots'></span>";
			}

			if ((($end_loop - 1) == $cur_page) || (($end_loop - 1) == $i)) {
				$dotclass1 = '';
			} else {
				$dotclass1 = "<span class='dots'></span>";
			}

			if ($cur_page == $i) {
				$pag_container .= "<li p='$i' class = 'selected' ><a href='" . esc_url($current_site_url . $i . '/') . "'>{$i}</a></li>";
			} elseif (($i == $start_loop) && ($cur_page != $i)) {

				$pag_container .= "<li p='$i' class='active'><a href='" . esc_url($current_site_url . $i . '/') . "'>{$i}</a></li>" . $dotclass;
			} elseif ($i == $end_loop) {

				$pag_container .= $dotclass1 . "<li p='$i' class='active'><a href='" . esc_url($current_site_url . $i . '/') . "'>{$i}</a></li>";
			} else {

				if (($i <= ($cur_page + 1)) && ($i >= ($cur_page - 1))) {

					$pag_container .= "<li p='$i' class='active'><a href='" . esc_url($current_site_url . $i . '/') . "'>{$i}</a></li>";
				} else {
					continue;
				}
			}
		...
		if ($next_btn && $cur_page < $no_of_paginations) {
			$nex = $cur_page + 1;
			$pag_container .= "<li p='$nex' class='active'><a href='" . esc_url($current_site_url . $nex . '/') . "'>Next</a></li>";
		} else if ($next_btn) {
			$pag_container .= "<li class='inactive'>Next</li>";
		}

		if ($last_btn && $cur_page < $no_of_paginations) {
			$pag_container .= "<li p='$no_of_paginations' class='active'><a href='" . esc_url($current_site_url . $no_of_paginations . '/') . "'>Last</a></li>";
		} else if ($last_btn) {
			$pag_container .= "<li p='$no_of_paginations' class='inactive'><a href='" . esc_url($current_site_url . $no_of_paginations . '/') . "'>Last</a></li>";
		}

		$pag_container = $pag_container . '
		</ul>
	</div>';
		echo wp_kses_post('<div class = "cvf-pagination-content">' . $msg . '</div>');
		...
		if (! empty($firnalposts)) {
			echo '<div class = "cvf-pagination-nav">' . wp_kses($pag_container, $whitelist_tags) . '</div>';
		} else {
